  ...
  for tabidx, tabnr in ipairs(tabnrs) do
    ...
    for windownr, windowid in ipairs(windownrs) do
      ...
      if #element.path_start > 0 and #element.path_end > 0 then
        -- filter out neo-tree.  Need to either add nvimtree or make this
        -- configurable
        if string.find(path_end, "neo-tree filesystem", 0, true) == nil then
          table.insert(windows, element)

          -- we only need one window per tab
          break
        end
      end
    end
